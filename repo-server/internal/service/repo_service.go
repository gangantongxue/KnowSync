...
func (s *RepoService) CreateRepo(ctx context.Context, ownerID, name, visibility, desc string) (*schema.Repo, error) {
	repo := &schema.Repo{
		OwnerID:     ownerID,
		Name:        name,
	...
	if err := s.collabRepo.AddCollaborator(ctx, collaborator); err != nil {
...
// GetRepo 获取知识库详情.
func (s *RepoService) GetRepo(ctx context.Context, repoID, userID string) (*schema.Repo, string, error) {
	if err := CheckRepoPermission(ctx, s.repoRepo, s.collabRepo, s.followRepo, repoID, userID, "VIEWER"); err != nil {
		return nil, "", err
	}
	...
	myRole := ""
	roleStr, err := s.collabRepo.GetUserRole(ctx, repoID, userID)
	if err == nil {
		myRole = roleStr
	}
...
func (s *RepoService) UpdateRepo(ctx context.Context, repoID, userID, name, desc, visibility string) (*schema.Repo, error) {
	if err := CheckRepoPermission(ctx, s.repoRepo, s.collabRepo, s.followRepo, repoID, userID, "ADMIN"); err != nil {
		return nil, err
	}
...
func (s *RepoService) DeleteRepo(ctx context.Context, repoID, userID string) error {
	if err := CheckRepoPermission(ctx, s.repoRepo, s.collabRepo, s.followRepo, repoID, userID, "ADMIN"); err != nil {
		return err
	}

	if err := s.repoRepo.DeleteRepo(ctx, repoID); err != nil {
...
func (s *RepoService) ListPublicRepos(ctx context.Context) ([]schema.Repo, error) {
	repos, err := s.repoRepo.ListPublicRepos(ctx)
	if err != nil {
