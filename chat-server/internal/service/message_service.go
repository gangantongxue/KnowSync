...
func (s *MessageService) SendPrivateMessage(ctx context.Context, senderID, receiverID, contentType, content, extra, replyToID string) (*schema.Message, error) {
	// 生成会话ID（排序后拼接）
...
func (s *MessageService) GetMessageByID(ctx context.Context, messageID string) (*schema.Message, error) {
	return s.Repo.Message.GetMessageByID(ctx, messageID)
}
...
func (s *MessageService) GetUserUnreadCounts(ctx context.Context, userID string) (map[string]int32, error) {
...
func (s *MessageService) GetUnreadCounts(ctx context.Context, userID string, conversations []ConversationInfo) (map[string]int32, error) {
...
func (s *MessageService) ForwardMessage(ctx context.Context, senderID, targetConversationType, targetConversationID string, messageIDs []string) (*schema.Message, error) {
	// 验证发送者是否在目标会话中
...
func (s *MessageService) getUserNames(ctx context.Context, userIDs []string) (map[string]string, error) {
...
func (s *MessageService) pushNewMessageEvent(userID string, msg *schema.Message) {
	data := map[string]any{
		"id":                msg.ID,
...
func (s *MessageService) pushGroupNewMessageEvent(ctx context.Context, groupID, senderID string, msg *schema.Message) {
...
func (s *MessageService) pushRecallEvent(ctx context.Context, msg *schema.Message, senderID string) {
...
func (s *MessageService) pushRecallToUser(userID, messageID, conversationType, conversationID string) {
	payload, err := json.Marshal(map[string]any{
...
func (s *MessageService) updatePrivateConversationLastMessageAt(ctx context.Context, senderID, targetConversationID string, now int64) {
	parts := strings.Split(targetConversationID, "_")
	if len(parts) != 2 {
		return
	}
...
func (s *MessageService) validateReplyMessage(ctx context.Context, conversationType, conversationID, replyToID string) error {
