...
// Info 返回工具元信息
func (g *GetFileContent) Info(ctx context.Context) (*schema.ToolInfo, error) {
...
// InvokableRun 执行工具调用
func (g *GetFileContent) InvokableRun(ctx context.Context, arguments string, opts ...tool.Option) (string, error) {
...
func (g *GetFileContent) execute(ctx context.Context, paramsJSON string) (string, error) {
	...
	if params.RepoID == "" {
		return `{"content": "", "message": "repo_id 不能为空"}`, nil
	}
	if params.FilePath == "" {
		return `{"content": "", "message": "file_path 不能为空"}`, nil
	}

	userID, _ := ctx.Value(CtxKeyUserID).(string)
	if userID == "" {
		return `{"content": "", "message": "无法获取用户信息"}`, nil
	}

	// 获取仓库详情（含 owner_id，同时验证权限）
	repo, err := g.repoDetailClient.GetRepo(ctx, params.RepoID, userID)
	if err != nil {
		return fmt.Sprintf(`{"content": "", "message": "获取仓库信息失败: %s"}`, err.Error()), nil
	}

	content, err := g.fileClient.GetFileContent(ctx, repo.OwnerID, params.RepoID, params.FilePath)
	if err != nil {
		return fmt.Sprintf(`{"content": "", "message": "读取文件内容失败: %s"}`, err.Error()), nil
	}

	if content == "" {
		return `{"content": "", "message": "文件内容为空"}`, nil
	}
